...
def menu():
    ...
    while True:                                                                                                         #bucle infinito
        try:                                                                                                            #try-catch(control de errores)
            return int(input("Introduce la opción(1, 2, 3, 4, 5 o 0): "))                                               #introducir una opción (1,2,3,4,5)
            break                                                                                                       #si el número introducido es entero se sale del bucle infinito
        except ValueError:                                                                                              #si se produce un ValueError
            print('\nError. Debe introducir un número entero! (del 0 al 5 el programa realizará diferentes funciones)') #mensaje de error y se continúa en el bucle infinito
...
from proyectouc4 import parcela, cultivo, asignacion, pasodias, infosist, mostrardics, datos
opmenu = 1
...
while opmenu != 0:
    ...
    if opmenu == 1:   #si introduce el número 1 en el menú inicial se activará la función parcela()
        ...
        if salir == 'S': #En caso afirmativo, se mostrarán los dics y se finalizará el programa    
            ...
            opmenu = 1
    elif opmenu != 5: #si el usuario introduce el número distinto a los establecidos devolverá un mensaje de error y se volverá al menú principal
        print('\nError. Número incorrecto. El programa realiza distintas funciones introduciendo números del 0 al 5. \n\nVuelve a intentarlo:')
